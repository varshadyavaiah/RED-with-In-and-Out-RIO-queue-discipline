...
/* m_idle,m_idletime*/
/*Same as DoDequeue function of RED in ns3 */
Ptr<QueueDiscItem>
RioQueueDisc::DequeueRED (void)
{
  NS_LOG_FUNCTION (this);
...
{
  Ptr<QueueDiscItem> p=DequeueRED();
  if (p != 0) {
		/* insert ns3 euivalent */
...
/* 
          ...
          if (dropType == DTYPE_UNFORCED) { //why aren't they reducing nqueued? copied from red code
